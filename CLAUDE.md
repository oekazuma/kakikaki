...
- `Splash.svelte`: 起動直後の覆い。iPad のホーム画面アプリは起動直後に縦向きのままの座標系で描かれ（拡大・ずれて見える）、その間は fixed の覆いもずれるため、ロゴを置かず背景色だけを画面より大きく敷く。`measure()` しながら画面サイズが 200ms 変わらず・フォントが読めたら（最短 250ms・最長約 2.5 秒）フェードで外す。ロゴ入りの起動画面は iOS が manifest のアイコンと background_color から生成するものに任せる。
...
判定の手触りは iPad 実機で決めるものとして、`JUDGE`（特に先読み `K`）、`RECOG`、`Canvas.svelte` の おてほんなし自動判定（4 秒）を定数にまとめてある。なぞる は子どもが「厳しい」と感じたので 2026-09 に緩めた（線から 12、先読み 8 点、終点の手前 4 点で可）。おてほんなし では左に `Sample`（書く文字と画数）を出す。
