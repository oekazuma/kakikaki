...
判定の手触りは iPad 実機で決めるものとして、`JUDGE`（特に先読み `K`）、`RECOG`、`Canvas.svelte` の おてほんなし自動判定（4 秒）を定数にまとめてある。なぞる は子どもが「厳しい」と感じたので 2026-09 に緩めた（線から 12、先読み 8 点、終点の手前 4 点で可）。おてほんなし では盤面の左上に `Sample`（画数と「？」）を重ね、字は見せない（右の「みる」も出さない。左カラムに置くと 1024×768 で溢れる。右上は右手で隠れる）。
