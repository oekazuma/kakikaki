...
- `recognize.ts`: 現在の言語の全文字のお手本を N 点に再サンプリング・重心合わせしたテンプレート（`Canvas` が `templatesFor(strokes)` で文字セットごとに 1 回だけ作り、`Tracer` が必ず渡す）と、書いた画列を照合して距離を取る。画の対応は書き順を問わず最も近い画を貪欲に当て、画数が違うときは画の切れ目を無視した全体の点列どうしの距離（`WHOLE` の下駄付き）も試して小さい方を採る（つなげ書き・途中で離した書き方を通すため）。前の画の上をなぞり直しただけの画（`RETRACE` 以内、長さ半分以上）は照合前に捨てる。`passes` は「1 位が目標で距離が `D_MAX` × 2 未満」または「2 位以内かつ 1 位との差が `MARGIN` 未満」。しきい値は `RECOG`。
...
判定の手触りは iPad 実機で決めるものとして、`JUDGE`（特に先読み `K`）、`RECOG`、`Canvas.svelte` の おてほんなし自動判定（4 秒）を定数にまとめてある。なぞる は子どもが「厳しい」と感じたので 2026-09 に緩めた（線から 12、先読み 8 点、終点の手前 4 点で可）。おてほんなし では盤面の左上に `Sample`（画数と「？」）を重ね、字は右の「みる」を押したときだけ 2 秒見せる（左カラムに置くと 1024×768 で溢れる。右上は右手で隠れる。字を常に出すと「おてほんなし」の趣旨と食い違う）。
