...
<p align="center">
  <img src="docs/intro.gif" alt="なぞる・じぶんで かく・おてほんなし の 3 モード、ひらがな・カタカナ・英語の切り替え、クイズ・メダル・使う人を 45 秒で紹介する動画" width="720">
</p>
...
- **ひらがな・かたかな・かんじ・えいご**: ホーム上部のトグルで切り替え。画面の色（青・緑・赤・オレンジ）が変わり、同じ単語を ひらがな（81 文字）・カタカナ（81 文字）・英語（A〜Z・a〜z の 52 文字。単語は Dog のように先頭が大文字）で練習できる。かんじ（小学 1〜6 年の 1026 字。学年ごとに字のマスが並び、1 字ずつ読みと一緒に練習。画面の色は赤）だけは単語ではなく字と読みの練習。記録・星・メダル・クイズの成績は人ごと・ことばごとに別に保存。
...
- **たくさんの単語をカテゴリから選ぶ**: どうぶつ・しぜん・くだもの・みのまわり・からだ・きもち・ひと・たべもの・あそび・やさい・のりもの（易しい順）。カードには表示していない残り 2 つの表記（例: ひらがな のときは カタカナと英語）を併記。「もじから えらぶ」で 1 文字ずつの練習もできる。
- **クイズ**: よみクイズ（もじを見てイラストを選ぶ／イラストを見てもじを選ぶ／きいてイラストを選ぶ／「り」で はじまる ものを選ぶ／穴あきの文字を選ぶ、の 5 形式を混ぜて 10 問）と かきクイズ（イラストを見て、聞いて、または単語の 1 文字だけ ？ になった穴埋めを、お手本なしで書く。5 問）。単語の文字数で かんたん・ふつう・むずかしい の 3 段階。
- **めだる と きろく**: 文字・単語・カテゴリごとの進捗率と、たくさんのメダル（行マスター、カテゴリはかせ、クイズ、練習した日数・連続した日数など）。実績画面には今月のカレンダーと「つづけて n にち」。条件を満たした瞬間に練習画面でお知らせ。
