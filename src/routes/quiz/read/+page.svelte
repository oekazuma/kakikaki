			...
			<section class="q" in:fly={{ x: 60, duration: 300 }}>
				{#if q.kind === 'word'}
					<div class="prompt card">
						<button class="hear" onclick={() => say(nameOf(q.answer), info().speech)} aria-label="きく"><Icon name="speaker" size={26} /></button>
						<b class="word">{nameOf(q.answer)}</b>
						<span>は どれ？</span>
					</div>
					<div class="choices">
						{#each q.choices as w (w.id)}
							<button class={['card', 'choice', { hit: hit === w.id, wrong: wrong.includes(w.id) }]} disabled={wrong.includes(w.id)} onclick={(e) => pick(w, e)}>
				...
				{:else}
					...
					<div class="choices">
						{#each q.choices as w (w.id)}
							<button class={['card', 'choice', 'text', { hit: hit === w.id, wrong: wrong.includes(w.id) }]} disabled={wrong.includes(w.id)} onclick={(e) => pick(w, e)}>
								{nameOf(w)}
							</button>
						{/each}
					</div>
...
<style>
	...
	.choice {
		...
		transition: transform 0.15s, opacity 0.3s, border-color 0.2s;
	}
	.choice img {
		height: 200px;
	}
