...
<section class="card">
  ...
  <ol>
    ...
    <li>
      単語カードか「もじから えらぶ」で文字を選びます。文字ごとに <b>なぞる（2 回）→ じぶんで かく</b>
      と自動で進み、単語の全文字を終えると完了画面の<b>「おてほんなしに ちょうせん」</b>から おてほんなし
      に挑戦できます（上のモード切替から選ぶこともできます）。
    </li>
  </ol>
  <dl>
    <dt>なぞる</dt>
    ...
    <dd>
      右の「きく」はいま書いている 1
      文字を、左の単語カードのスピーカーは単語全体を読み上げます。音声はこれらのボタンでしか流れません。
    </dd>
  </dl>
</section>
...
<section class="card">
  ...
  <ul>
    ...
    <li>
      トロフィー画面には今月のカレンダーと「つづけて n にち」が出ます。連続日数は ひらがな・かたかな・えいご
      をまたいで数え、今日まだ練習していなくても前日まで続いていれば切れません。
    </li>
    <li>
      クイズは よみ（10 問。文字→絵、絵→文字、聞いて選ぶ、頭文字、穴埋め の 5 形式）と かき（5
      問。絵を見て書く・聞いて書く を交互に出します）。単語の文字数で かんたん・ふつう・むずかしい
      に分かれ、正解数を記録します。
    </li>
    <li>
...
<section class="card">
  ...
  <p>
    ...
    「苦手な文字」（おてほんなしで 2 回以上まちがえた文字と、じぶんでかくが星 1
...
<section class="card">
  ...
  <ul>
    ...
    <li>
      ホーム画面のアイコンを削除すると、記録も一緒に消えます。右の「バックアップ」でファイルに書き出しておけば、あとで読み込めます。
    </li>
    <li>
