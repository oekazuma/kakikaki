...
<button class="card" style:width="{size}px" {onclick}>
  {#if wordCrown(word.name)}<span class="badge">👑</span>{:else if wordStar(word.name)}<span class="badge">⭐</span
    >{/if}
  {#if missing}
...
<script lang="ts">
  ...
  const MODES: { id: Mode; label: string; hint: string; title: string }[] = [
    ...
    },
    {
      id: 'test',
      label: '🌟 おてほんなし',
      hint: 'おてほんを みないで かいてみよう。かけたら「できた」',
...
<main>
  ...
  <section class="card parent">
    ...
    <p>
      ...
      3.0）を使用しています。練習記録はこの iPad の中にだけ保存されます。
    </p>
